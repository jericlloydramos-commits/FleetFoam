...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'CLAIM_REQUEST') {
      const { crewId, crewEmail, crewName } = body;
      jobs[jobIndex].claim_requested_by = crewId;
      jobs[jobIndex].claim_status = 'PENDING';
      jobs[jobIndex].claim_requested_at = new Date().toISOString();
      jobs[jobIndex].claim_requester = {
        id: crewId,
        email: crewEmail || 'crew@fleetfoam.com',
        name: crewName || 'Crew Specialist',
        role: 'CREW',
        status: 'ACTIVE',
      };
      saveJobsFile(jobs);
    ...
    if (action === 'CLAIM_ACCEPT') {
      const crewId = jobs[jobIndex].claim_requested_by;
      const crew = jobs[jobIndex].claim_requester;
      jobs[jobIndex].assigned_to = crewId || undefined;
      jobs[jobIndex].assignee = crew || undefined;
      jobs[jobIndex].claim_status = 'ACCEPTED';
      jobs[jobIndex].updated_at = new Date().toISOString();
      saveJobsFile(jobs);
      return NextResponse.json({ success: true, job: jobs[jobIndex] });
    }

    if (action === 'CLAIM_DENY') {
      jobs[jobIndex].claim_status = 'NONE';
      jobs[jobIndex].claim_requested_by = undefined;
      jobs[jobIndex].claim_requester = undefined;
      saveJobsFile(jobs);
      return NextResponse.json({ success: true, job: jobs[jobIndex] });
    }

    if (action === 'DELETE_JOB') {
      const filtered = jobs.filter((j) => j.id !== jobId && j.booking_id !== body.bookingId);
      saveJobsFile(filtered);
